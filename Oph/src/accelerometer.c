...
// functions from telemetry-uplink for sending telemetry data
// to the server to be downlinked
#include "send_telemetry.h"
#include "socket.h"

// used for making dummy accl data
#include <math.h>
#include <time.h>
...
void accelerometer_create_output_folders(char *base_output_folder,
  ...
  snprintf(base_output_folder, 256, "%s/%s", config.accelerometer.output_dir,
           folder_name);
...
FILE *accelerometer_open_new_file(const char *output_folder, int chunk_number,
                                  double start_time) {
  char filename[256];
  sprintf(filename, "%s/%.6f_chunk_%04d.bin", output_folder, start_time,
          chunk_number);
  FILE *file = fopen(filename, "wb");
  if (file == NULL) {
    char error_msg[512];
    snprintf(error_msg, sizeof(error_msg), "Error opening file: %s",
             strerror(errno));
...
int accelerometer_init() {
  ...
  if (inet_pton(AF_INET, config.accelerometer.raspberry_pi_ip,
                &accel_data.serv_addr.sin_addr) <= 0) {
  ...
  while (retry_count < MAX_RETRIES) {
    if (connect(accel_data.sock, (struct sockaddr *)&accel_data.serv_addr,
                sizeof(accel_data.serv_addr)) < 0) {
      char error_msg[256];
      snprintf(error_msg, sizeof(error_msg),
               "Connection Failed. Retrying... (%d/%d)", retry_count + 1,
               MAX_RETRIES);
      accelerometer_log_message(error_msg);
      sleep(1);
      retry_count++;
  ...
  if (pthread_create(&accel_data.thread_id, NULL, accelerometer_run, NULL) !=
      0) {
...
void *accelerometer_run(void *arg) {
...
#ifdef SEND_DUMMY_ACCL
  int socketfd = make_async_connected_send_socket("localhost", "3000");
  struct timespec now;
  ...
  while (accel_data.keep_running) {
    ...
    double dummy_accl_val = 10 * sin((2 * M_PI / 10) * (now.tv_sec + now.tv_nsec / 1e9));
    // printf("Sending dummy accelerometer data: %.2f\n", dummy_accl_val);
    send_sample_double(socketfd, "accl", now.tv_sec, dummy_accl_val);
#endif
    ...
    if (incomplete_line[0] != '\0') {
      ...
      if (newline) {
        ...
        if (sscanf(incomplete_line, "%d,%lf,%lf,%lf,%lf", &accel_id, &timestamp,
                   &x, &y, &z) == 5) {
          accel_id--; // Convert to 0-based index
          if (accel_id >= 0 &&
              accel_id < config.accelerometer.num_accelerometers) {
            if (start_times[accel_id] == 0)
              start_times[accel_id] = timestamp;

            if (current_files[accel_id] == NULL) {
              chunk_start_times[accel_id] = timestamp;
              current_files[accel_id] = accelerometer_open_new_file(
                  output_folders[accel_id], chunk_numbers[accel_id],
                  chunk_start_times[accel_id]);
            }
            ...
            if (samples_received[accel_id] %
                    config.accelerometer.print_interval ==
                0) {
              time(&current_time_t);
              double elapsed_time = difftime(current_time_t, start_time_t);
              double average_rate = samples_received[accel_id] /
                                    (timestamp - start_times[accel_id]);

              char status_msg[512];
              snprintf(status_msg, sizeof(status_msg),
                       "Accelerometer %d - Samples: %ld | Elapsed time: %.0f s "
                       "| Avg rate: %.2f Hz",
                       accel_id + 1, samples_received[accel_id], elapsed_time,
                       average_rate);
              accelerometer_log_message(status_msg);
            }

            if (timestamp - chunk_start_times[accel_id] >=
                config.accelerometer.chunk_duration) {
              fclose(current_files[accel_id]);
              chunk_numbers[accel_id]++;
              current_files[accel_id] = accelerometer_open_new_file(
                  output_folders[accel_id], chunk_numbers[accel_id], timestamp);
              chunk_start_times[accel_id] = timestamp;
            }
          }
        }
    ...
    while ((line_end = strchr(line_start, '\n')) != NULL) {
      *line_end = '\0'; // Temporarily replace newline with null terminator
      ...
      int accel_id;
      double timestamp, x, y, z;
      if (sscanf(line_start, "%d,%lf,%lf,%lf,%lf", &accel_id, &timestamp, &x,
                 &y, &z) == 5) {
        accel_id--; // Convert to 0-based index
        if (accel_id >= 0 &&
            accel_id < config.accelerometer.num_accelerometers) {
...
void accelerometer_shutdown() {
  accelerometer_log_message(
      "Attempting to shut down accelerometer data collection");
...
void accelerometer_get_status(AccelerometerStatus *status) {
  if (status == NULL) {
    accelerometer_log_message(
        "Error: Null pointer passed to accelerometer_get_status");
    return;
  }
