...
def get_data_folders():
    ...
    return [f for f in folders if f.endswith('_BVEX_data')]

def get_latest_files(sftp, folder):
    files = sftp.listdir(os.path.join(REMOTE_DATA_PATH, folder))
    spectrum_files = [f for f in files if f.endswith('_spectrum_data.txt')]
    power_files = [f for f in files if f.endswith('_integrated_power_data.txt')]
...
def key_press_handler(event):
    ...
    if event.key == 'q':
        ...
        plt.savefig(f"spectrum_capture_{timestamp}.png", dpi=300)
        print(f"Plot saved as spectrum_capture_{timestamp}.png")
...
def plot_data(selected_folder):
    ...
    ax2.set_ylabel('Integral Power (arb dB.)')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--help':
        print("Usage: python read_latest_data.py [OPTIONS]")
        print("Options:")
        print("  --display-time SECONDS   Set the display window time (default: 20 seconds)")
        print("  --update-interval MS     Set the plot update interval (default: 50ms)")
