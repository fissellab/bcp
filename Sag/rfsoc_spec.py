...
def get_vacc_data(fpga, nchannels, nfft):
    try:
        ...
        if spectrum_file:
            spectrum_file.write(f"{timestamp} " + " ".join(map(str, 10*np.log10(np.fft.fftshift(np.flip(spectrum_data))))) + "\n")
...
def start_data_acquisition(fpga, mode, num_channels, num_fft_points):
    ...
    while running:
        ...
        # Perform a filesystem sync every 60 seconds
        current_time = time.time()
        if current_time - last_sync_time > 60:
            os.sync()
            last_sync_time = current_time

        rotate_files()  # This will now check if it's time to rotate files
